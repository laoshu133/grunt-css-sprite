...
function createBySliceData(sliceData, options, gruntFileApi, callback) {
    var imgSize = sliceData.spriteImgData.properties;
    var retinaImgSize = [2 * imgSize.width, 2 * imgSize.height];
    ...
    spritesmith.Layout.addAlgorithm('xy-wh', {
        ...
        placeItems: function(items) {
            ...
            items.forEach(function(item) {
                var retinaImgPath = item.meta.img._filepath;
                var filename = path.basename(retinaImgPath);
                var coord = retinaImgHash[retinaImgPath];
    ...
    var beforePlace = path.join(options.spritedest, options.IMAGE_SET_PLACE_FILE_BEFORE);
    var endPlace = path.join(options.spritedest, options.IMAGE_SET_PLACE_FILE_END);
